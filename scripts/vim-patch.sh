...
get_vim_patch() {
  ...
  local vim_full
  vim_full="$(git --no-pager show --no-color -1 --pretty=medium "${vim_commit}" \
...
list_vim_patches() {
  ...
  for vim_commit in ${vim_commits}; do
    ...
    if [[ -n "${vim_tag}" ]]; then
      ...
      if (cd "${VIM_SOURCE_DIR}" && git --no-pager show --no-color --name-only "v${vim_commit}" 2>/dev/null) | grep -q ^runtime; then
